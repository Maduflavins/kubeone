...
func BytesToKubeOneCluster(cluster, tfOutput, credentialsFile []byte) (*kubeoneapi.KubeOneCluster, error) {
	...
	// Decode the bytes into the internal cluster object
	// The bytes will be unmarshalled into the correct external version, defaulted, and converted
	// into the internal version
	if err := runtime.DecodeInto(kubeonescheme.Codecs.UniversalDecoder(), cluster, internalCluster); err != nil {
		return nil, err
	}

	return DefaultedKubeOneCluster(internalCluster, tfOutput, credentialsFile)
}

// DefaultedKubeOneCluster sources information from Terraform, applies dynamic defaults, and validates
// the cluster object
func DefaultedKubeOneCluster(internalCluster *kubeoneapi.KubeOneCluster, tfOutput, credentialsFile []byte) (*kubeoneapi.KubeOneCluster, error) {
	if tfOutput != nil {
		if err := SourceKubeOneClusterFromTerraformOutput(tfOutput, internalCluster); err != nil {
...
func SourceKubeOneClusterFromTerraformOutput(terraformOutput []byte, cluster *kubeoneapi.KubeOneCluster) error {
